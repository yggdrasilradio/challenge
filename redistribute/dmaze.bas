...
11PRINT"$ ";:ATTR1,0:PRINT"Gold":PRINT:ATTR2,0:PRINT"* Use arrow keys to move.":PRINT"* Press 'T' to teleport.":PRINT"* Avoid the dragons.":PRINT"* Collect all the gold to win.":PRINT"* Collect swords to fight the dragons."
12PRINT"* One sword will kill one dragon.":PRINT"* Swords magically vanish after use.":PRINT"* Good luck!":ATTR1,0:LOCATE0,23:PRINT"Generating maze:";:MW=19:MH=11:T=MW*MH:DIMA$(MH*2+1):R1$="+":R2$="!":FORI=1TO MW
13R1$=R1$+"---+":R2$=R2$+" X !":NEXTI:FORI=1TO MH*2+1:IFI AND1 THENA$(I)=R1$ ELSEA$(I)=R2$
...
105IFS$="T" THENNX=RND(MW)*4-1:NY=RND(MH)*2
106C$=MID$(A$(NY),NX,1):IFC$="$" THENS=S+10:LOCATE0,23:PRINT"Gold:";S;:MID$(A$(NY),NX,1)=" ":M$="Found gold":GOSUB9000
107IFC$="^" THENI$=I$+"^":LOCATE41,23:ATTR1,0:PRINTLEN(I$);:MID$(A$(NY),NX,1)=" ":M$="Found sword":GOSUB9000
108IFINSTR("!+-",C$)=0 THENLOCATEX-1,Y-1:PRINT" ";:LOCATENX-1,NY-1:ATTR0,0:PRINT"O";:X=NX:Y=NY
109IFS=100 THENGOSUB6000
110GOSUB2000:GOTO100
